...
public class RapportsActiviteController(
    ...
    OperationalAccessService accessService,
    IWebHostEnvironment env) : Controller
{
    ...
    public async Task<IActionResult> ExportPdf(Guid id)
    {
        ...
        var (logoBytes, _) = ResolveLogo(rapport.Activite.Groupe?.LogoUrl);
        var bytes = RapportActiviteExport.BuildPdf(rapport, logoBytes);
    ...
    public async Task<IActionResult> ExportWord(Guid id)
    {
        ...
        var (logoBytes, logoMime) = ResolveLogo(rapport.Activite.Groupe?.LogoUrl);
        var html = RapportActiviteExport.BuildWordHtml(rapport, logoBytes, logoMime);
        return File(Encoding.UTF8.GetBytes(html), "application/msword", $"{BuildSafeFileName(rapport.Activite.Titre)}-rapport.doc");
    }

    /// <summary>
    /// Resout le logo du groupe (ou le logo par defaut du district) en octets pour l'en-tete du rapport.
    /// </summary>
    private (byte[]? Bytes, string? Mime) ResolveLogo(string? logoUrl)
    {
        var path = ResolvePhysicalPath(BranchDisplay.LogoOrDefault(logoUrl))
                   ?? ResolvePhysicalPath(BranchDisplay.DefaultLogoUrl);
        ...
        return (System.IO.File.ReadAllBytes(path), mime);
    }

    private string? ResolvePhysicalPath(string? webUrl)
    {
        if (string.IsNullOrWhiteSpace(webUrl) || string.IsNullOrWhiteSpace(env.WebRootPath))
        {
            return null;
        }

        var relative = webUrl.Split('?', '#')[0].TrimStart('/').Replace('/', Path.DirectorySeparatorChar);
        var root = Path.GetFullPath(env.WebRootPath);
        var full = Path.GetFullPath(Path.Combine(root, relative));
        // Empeche tout echappement hors de wwwroot via des chemins relatifs.
